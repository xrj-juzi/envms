...
abstract class Base implements \IteratorAggregate
{
     ...
     * @throws Exception
     */
    private function executeQuery($parameters, $startTime, $execTime)
    {
        try {
            if ($this->result->execute($parameters) === true) {
                ...
                $message = "SQLSTATE: {$error[0]} - Driver Code: {$error[1]} - Message: {$error[2]}";
                $this->result = false;
                Log::SqlErrorLog($exception->getMessage(),$this->buildQuery(),$parameters);
                throw new Exception("PDO Excute faild(see error log): ".$message);
            }
        }catch (\PDOException $exception){
            Log::SqlErrorLog($exception->getMessage(),$this->buildQuery(),$parameters);
            throw new Exception("PDO Excute faild(see error log): ".$exception->getMessage());
        }
    }
